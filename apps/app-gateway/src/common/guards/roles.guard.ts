...
export class RolesGuard implements CanActivate {
  ...
  constructor(
    ...
    @Inject(PermService) private readonly permService: PermService,
    private readonly config: ConfigService,
  ) {
    // this.globalWhiteList = [].concat(this.config.get('perm.router.whitelist') || [])
    this.config.get('perm.router.whitelist').forEach(wo=>{
      this.globalWhiteList.push(this.config.get('app.prefix')+wo.path)
    })
  }
